#!/bin/bash
# Validates PRD infrastructure plan JSON structure (wave-based schema)

set -e

PLAN_FILE="${1:-/tmp/.prd_infra_plan.json}"

if [[ ! -f "$PLAN_FILE" ]]; then
  echo "{\"status\":\"error\",\"error\":\"Infrastructure plan file not found: $PLAN_FILE\"}"
  exit 1
fi
...
REQUIRED_FIELDS=("prd" "totalTasks" "completeTasks" "remainingTasks")
